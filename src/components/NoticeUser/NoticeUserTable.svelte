<script lang="ts">
    ...
    const deleteUserNotice = (setId: number) => {
        ...
        popup("정말 삭제하시겠습니까?", 3, async (data: boolean) => {
            ...
            const response = await got("/notice/user", "DELETE", params)
...
{#if tableFixed}
    {#each tableFixed as item, index}
        <tr>
            ...
            <td><button class="small red" type="button" on:click={()=>{deleteUserNotice(item["id"])}}>삭제</button></td>
