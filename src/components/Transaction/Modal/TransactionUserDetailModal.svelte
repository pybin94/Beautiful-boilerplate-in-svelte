<script lang="ts">
    ...
    const updateTransactionStatus = async () => {
        ...
        const response = await got(`/user/transaction`, "POST", params)
...
<form on:submit|preventDefault={updateTransactionStatus}>
    <div class="form-body">
        <div class="form-group">
            <p>에이전트 이름</p>
            <input type="text" disabled value={transactionInfo["toId"]["nickname"]}>
        </div>
        <div class="form-group">
            <p>에이전트 아이디</p>
            <input type="text" disabled value={transactionInfo["toId"]["identity"]}>
        </div>
        <div class="form-group">
            <p>신청금액</p>
