<script lang="ts">
    ...
    export let handleSearchUserVisible: any;
    ...
    const sendMessage = async (sendMessage: SendMessage) => {
        ...
        let params = {
            ...
            title,
            contents,
            direction: sendMessage.direction,
            status: 0,
            isBroadcast: 0,
        }
        ...
        if(sendMessage.isBroadcast) {
            ...
            popup("쪽지를 전체전송 하시겠습니까?", 2, async (data: boolean) => {
                ...
                response = await got(`/notice/user/message`, "PATCH", params);
            })
        } else {
            response = await got(`/notice/user/message`, "PATCH", params);
        }
        ...
        <div class="form-group">
            ...
            <div class="message-target">
                ...
                <button 
                    class="line"
                    type="button"
                    on:click={handleSearchUserVisible}
                >플레이어 추가</button>
            </div>
        </div>
        <div class="form-group">
